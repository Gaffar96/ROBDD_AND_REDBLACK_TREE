...
class BlackRedTree
{
    ...
    {
    public:
        Node(value_type pai) : m_bIsRed(true), m_Val({pai.first, pai.second}), m_Key(pai.first), m_Data(pai.second)
        {
        }
        value_type m_Val;
        K m_Key;
        ...
        bool is4Node()
        {
            return m_Left != nullptr && m_Left->m_bIsRed && m_Right != nullptr && m_Right->m_bIsRed;
        }
        bool is2Node()
        {
            return !m_bIsRed && (m_Left == nullptr || !m_Left->m_bIsRed) && (m_Right == nullptr || !m_Right->m_bIsRed);
        }
...
public:
    ...
    void insert(value_type ma)
    {
        ...
        while (!h.isNull())
        {
            ...
            if (ma.first < h.node(h.NODE)->m_Val.first)
            {

                h.down(true);
            }
            else if (h.node(h.NODE)->m_Val.first < ma.first)
            {
                h.down(false);
            }
            else
        ...
        h.set(new Node({ma.first, ma.second}), h.NODE);
        h.split();
        this->m_Root->m_bIsRed = false;
    }
    ...
    {
    public:
        BlackRedTree *SuperKlasse = nullptr;
        const int NODE = 0;
        const int DAD = 1;
        const int G_DAD = 2;
        const int GG_DAD = 3;
        ...
        void init()
        {
            for (int counter = 0; counter < length; ++counter)
            {
                ...
                m_Nodes[counter] = nullptr;
            }
        }
        NodeHandler(Node *n, BlackRedTree *S_K) : SuperKlasse(S_K)
        {
            init();
            m_Nodes[NODE] = n;
        }
        ...
        void set(Node *n, int kind)
        {
            ...
            {
                SuperKlasse->m_Root = n;
            }
            else if (node(kind) != nullptr ? node(kind + 1)->m_Left == node(kind) : n->m_Val.first < node(kind + 1)->m_Val.first)
            {
...
iterator(Node* two,int a){
  m_Nodes2Visit.push_back(two);
}


void goDown(Node* pTmp) {
for(;pTmp;pTmp = pTmp->m_Left)
...
//class iterator : public std::iterator<std::forward_iterator_tag,value_type> {
//public: // operator
friend bool operator==(const iterator& crArg1,const iterator& crArg2) {
return crArg1.m_Nodes2Visit == crArg2.m_Nodes2Visit;
}
friend bool operator!=(const iterator& crArg1,const iterator& crArg2) {
return crArg1.m_Nodes2Visit != crArg2.m_Nodes2Visit;
}
...
public:
    ...
    Func *ite(Func *i, Func *t, Func *e)
    {
        ...
        else
        {
            ...
            if(res == m_Unique.end())
            {
                seco=new Func(ciVar, T, E);
                m_Unique.insert({entry,seco});
            }

            if(res == m_Unique.end())
            {
                m_Computed.insert({args,seco});
    ...
    Func *genVar(int i)
    {
        ...
        if(res == m_Unique.end())
        {
            ...
            se= new Func(i, genTrue(), genFalse());
            m_Unique.insert({entry, se});
        }
        else
        {
...
private:
    BlackRedTree<Triple, Func*> m_Unique;
    Func *m_pTrue = new Func(true);
    Func *m_pFalse = new Func(false);
    BlackRedTree<string, Func*> inp;
    vector<Func *> vecs;
    vector<string> funcs;
    vector<string> ou; //
    BlackRedTree<FuncTriple, Func *> m_Computed;
    BlackRedTree<Func, bool*> out2;
};
